...
use fhdl_netlist::netlist::{Module, Port};
...
impl<'tcx> Compiler<'tcx> {
    ...
    fn add_pin_constraint(
        ...
        let mut pins_map: FxHashMap<Port, Pin> = Default::default();
        ...
        for (ident, pins) in &constr.pins {
            ...
            loop {
                ...
                match (port, pin) {
                    (Some(port), Some(pin)) => {
                        if ctx.module.is_input(port) || ctx.module.is_output(port) {
                            ...
                            pins_map.insert(port, pin.clone());
                        } else {
