...
use fhdl_netlist::net_list::NodeOutId;
...
pub struct PinConstraints {
    pub vendor: Vendor,
    pub pins: FxHashMap<NodeOutId, Pin>,
}
...
impl<'tcx> Compiler<'tcx> {
    ...
    fn add_pin_constraint(
        ...
        let mut pins_map: FxHashMap<NodeOutId, Pin> = Default::default();
        ...
        for (ident, pins) in &constr.pins {
            ...
            loop {
                ...
                match (node, pin) {
                    (Some(node), Some(pin)) => {
                        if self.netlist.is_input(node) || self.netlist.is_output(node) {
                            ...
                            pins_map.insert(node, pin.clone());
                        } else {
    ...
    pub fn write_pin_constraints(&self, dir_path: impl AsRef<Path>) -> Result<(), Error> {
        for (name, pin_constr) in &self.pin_constr {
            self.write_pin_constraint(pin_constr, dir_path.as_ref(), name.as_str())?;
        }
    ...
    fn write_pin_constraint(
        ...
        match pin_constr.vendor {
            Vendor::Xilinx => {
                self.write_xilinx_constraint(&mut path, &pin_constr.pins)?;
            }
        }
    ...
    fn write_xilinx_constraint(
        ...
        for (node, pin) in pins {
            let sym = self.netlist[*node].sym.unwrap();
            let pin = &pin.name;

            writeln!(buffer, "set_property PACKAGE_PIN {pin} [get_ports {sym}]").unwrap();
            writeln!(
                buffer,
